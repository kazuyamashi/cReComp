...
class Fifo_8(object):
	...
	def gen_alw(self,flag,fo):
		...
		while i < int(flag.w_cycle_8)-1:
			...
			i = i + 1
		if int(flag.w_cycle_8) > 0:
			fo.write("\t\t\tSND_DATA_8_%s: 		state_8 <= CYCLE_END_8;\n"%(i))
		...
		if int(flag.r_cycle_8) > 0:
			fo.write("\t\t\tREADY_RCV_8: if(data_empty_8 == 0) 	rcv_en_8 <= 1;\n")
			fo.write("\t\t\tPOSE_8: rcv_en_8 <= 0;\n")
		if int(flag.w_cycle_8) > 0:
			fo.write("\t\t\tREADY_SND_8: 	if(data_full_8 == 0)	snd_en_8 <= 1;\n")
			fo.write("\t\t\tSND_DATA_8_%s: snd_en_8 <= 0;\n"%(self.w_times-1))
