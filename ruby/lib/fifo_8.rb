...
class FIFO8
			...
			if cur_r > 8 then
				...
				if reg2fifo[3] != nil
					...
					while n < reg2fifo[3].delete("x").to_i
						...
						fo.puts "reg [#{@bit_witdh_8_s[k + n].to_i-1}:0] #{@reg2fifo_stack_8_s[k + n]};\n"
					...
					fo.puts "reg [#{@bit_witdh_8_s[k].to_i-1}:0] #{@reg2fifo_stack_8_s[k]};\n"
					cur_s = cur_s + reg2fifo[1].to_i
					k = k + 1
				end
			end
	...
	def gen_alw(flag,fo)
		...
		if @reg2fifo_stack_8_r != nil
			while i < @reg2fifo_stack_8_r.count
				...
				fo.puts("\t\t#{@reg2fifo_stack_8_r[i]} <= rcv_data_8[#{bitmax}:#{bitmin}];\n")
				i = i + 1
		...
		if @reg2fifo_stack_8_s != nil
			while i < @reg2fifo_stack_8_s.count
				...
				fo.puts("assign snd_data_8[#{bitmax}:#{bitmin}] = #{@reg2fifo_stack_8_s[i]};\n")
				i = i + 1
				bitmin = bitmax.to_i + 1
			end
		end
	...
	def get_fifostack_r()
		return @reg2fifo_stack_8_r
	end

	def get_bitwidth_r()
		return @bit_witdh_8_r
	end

	def get_fifostack_s()
		return @reg2fifo_stack_8_s
	end
	def get_bitwidth_s()
		return @bit_witdh_8_s
	end
end